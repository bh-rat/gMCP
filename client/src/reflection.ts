...
export class DescriptorCache {
  ...
  private createReflectionPackageDefinition(): any {
    ...
    const candidates = [
      path.join(__dirname, '../../proto/grpc/reflection/v1alpha/reflection.proto'), // when running from client/dist
      path.join(process.cwd(), '../proto/grpc/reflection/v1alpha/reflection.proto'),  // when run from client directory
