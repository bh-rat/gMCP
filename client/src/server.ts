...
import { RealMcpClient as McpClient } from './real_client';
...
async function handleListTools(data: any, res: http.ServerResponse) {
  ...
  // Derive minimal JSON schema from grpc service/method
  const withSchemas = await Promise.all(tools.map(async (t: any) => ({
    ...t,
    inputSchema: await deriveSchemaFromServiceMethod(t.grpcService, t.grpcMethod, clientId)
  })));
  client.close();
  res.writeHead(200, { 'Content-Type': 'application/json' });
...
async function deriveSchemaFromServiceMethod(grpcService: string, grpcMethod: string, clientId: string): Promise<any> {
  // Use the actual schema derivation from the LLM integration
  const llmClient = llmClients.get(clientId);
  if (llmClient) {
    try {
      const { inputType } = await llmClient.mcpClient.deriveTypesFromService(grpcService, grpcMethod);
      const schema = await llmClient.mcpClient.getSchemaFromMessageType(inputType, grpcService);
      return schema;
    } catch (error) {
      console.error(`Failed to derive schema for ${grpcService}.${grpcMethod}:`, error);
    }
  }

  // Fallback to hardcoded schema for weather service
  if (grpcService === 'examples.weather.WeatherService') {
    if (grpcMethod === 'GetWeather') {
      return {
        type: 'object',
        properties: {
          location: { type: 'string', description: 'City or place', minLength: 1 },
          units: { type: 'string', enum: ['metric', 'imperial'], description: 'Unit system' }
        },
        required: ['location', 'units']
      };
    }
    if (grpcMethod === 'GetWeatherForecast') {
      return {
        type: 'object',
        properties: {
          location: { type: 'string', description: 'City or place', minLength: 1 },
          date: { type: 'string', description: 'Date in YYYY-MM-DD format', minLength: 1 },
          units: { type: 'string', enum: ['metric', 'imperial'], description: 'Unit system' }
        },
